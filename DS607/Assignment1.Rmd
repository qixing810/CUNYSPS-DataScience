...
It's my first impression that odor and color could be an easy way to detect whether a mushroom poisonous or not. Further analysis like feature selection would be necessary if the project is going on. 
```{r}
sub_mush <- mash_uci %>% 
...
##5.Take a look at the first 20 rows to make sure the abbreviations are being replaced
```{r}
head(sub_mush,n=20)
```
