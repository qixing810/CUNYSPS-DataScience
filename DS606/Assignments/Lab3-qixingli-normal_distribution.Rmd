...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    library(tidyverse)
    library(plotly)
    mhist <- mdims %>% ggplot(aes(hgt)) + 
      geom_histogram(color='blue', fill='blue',alpha=0.5) +
      labs(title = "Male")+
      xlab("Height")
    ggplotly(mhist)

    fhist <- fdims %>% ggplot(aes(hgt)) + 
    ...
    ```
    ```
    The distributions of the male and female heights are approximately symmetric 
    or normal distribute. And they are both multi-model.
    ```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
    ```
    Yes. It appears nearly normal distribution.
    ```
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
    ```{r}
    sim_norm <- rnorm(n = length(fdims$wgt), mean = mean(fdims$wgt), sd = sd(fdims$wgt))
    qqnorm(sim_norm)
    qqline(sim_norm)
    ```
    ```
    Most of the points lie on the line or very close to it.
    ```
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    ```
    Yes. The normal probability plot does look similar to the simulated data plots.
    So, we can say that the female heights are nearly normal.
    ```
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    ```
    What is the probability that a randomly chosen female is taller than 165cm?
    ```
    ```{r}
    1 - pnorm(q=165, mean = fhgtmean, sd = fhgtsd)
    ```
    ```
    What is the probability that a randomly chosen female is less than 55kg?
    ```
    ```{r}
    pnorm(q=165, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
    ``` 
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    ```
    This is likely due to the discrete scale on which the data was measured.
    ```
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    your findings.
    ```
    Based on the normal probability plot, it is right skewed.
    ```
    ```{r}
    qqnorm(fdims$kne.di)
    qqline(fdims$kne.di)
    ...
    ```{r}
    kne_hist <- fdims %>% ggplot(aes(kne.di)) + 
      geom_histogram(color='red', fill='red',alpha=0.5) +
      xlab("Knee Diameter")
    ggplotly(kne_hist)
    ```
