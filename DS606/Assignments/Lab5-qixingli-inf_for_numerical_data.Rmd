...
```
H0: There is no difference of average weights of babies between smoking and non-smoking mothers.
Ha: There is some difference of average weights of babies between smoking and non-smoking mothers.
```
...
```{r}
young_mom <-  range(nc[nc$mature == "younger mom",]$mage)
young_mom
mature_mom <-  range(nc[nc$mature == "mature mom",]$mage)
mature_mom
```
```
From the range, we can see that the bigest age of younger mom is 34 and the smallest age of mature mom is 35, so the cutoff for younger is less than or equal to 34, and mature is greater than 34.
```
...
plain language.
```
Does the average weights of babies differ between their genders?
```
```{r}
