...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    ```
    Streak length of 1 means there is only one hit in the streak. 
    There is 1 hit and 1 miss in a streak of 1. 
    A streak of 0 means there is only a miss in the streak.
    ```
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    ```
    From the bar chart, we can see the distribution is skewed to the right. 
    The typical streak was 0.
    The longest length of streak was 4.
    ```
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    ```
    ```{r echo=FALSE}
    table_unfair <- table(sim_unfair_coin)
    table_unfair[1]
    ```
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    `sim_basket`.
    ```
    Add the argument prob to the sample function.
    ```
    ```{r}
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    How long is the player's longest streak of baskets in 133 shots?<br>
    ...
    **The distribution is also skewed to the right.**
    ...
    <br>
    **The typical streak length for this simulated independent shooter is:**
    ```{r echo=FALSE}
    names(sort(-table(sim_streak)))[1]
    ```
    <br>
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    different? Explain your reasoning.<br>
    
    **
    I would expect a similar distribution which is skewed to the right. However,
    the streak lengths are longer than Kobe's.
    **<br>
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    shooting patterns? Explain.<br>
    
    **
    The distributions are very similar, both are skewed to the right, that is 0 streak is more typical,but the streak lengths are not the same. Looks like the the hot hand model does not fit Kobe's shooting patterns. As we we run the simulation many times, it is more clear that hot hand model fits the simulated shooter than Kobe.
    **
