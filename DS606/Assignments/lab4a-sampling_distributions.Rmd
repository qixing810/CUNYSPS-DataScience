...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    ```
    The sample distribution is skewed to the right. 
    It can represent the population distribution.
    ```
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?
    ```
    There are 5000 elements. 
    It's approximately normal distribution, 
    the highest frequency is approaching to the middle point. 
    If we can collecte 50,000 sample means, 
    it will get more and more close to the population mean.
...
```
When the sample size is larger, the center become more prominent and the spread smaller.
```
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    ```{r}
    sample_mean_price50 <- sample(price, 50)
    mean(sample_mean_price50)
    ```
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    ```
    The distribution 3 has a smaller spread. 
    I prefer a distribution with a small spread.
    ```
