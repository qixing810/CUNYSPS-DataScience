...
"typical" to mean.
```
The distribution of the sample is skewed to the right. 
The "typical" size is between 1000 to 1500 because it is the mode. 
The mean will be on the right most tail and the size is between 2000 and 2500.
```

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
```
No, I don't think the distributin would be identical, but probability some similarlity. 
Any given sample could select a wide range of values from the population, making any two samples extremely unlikely to be identical. 
```
...
```
Yes, it captures the true average size of houses in Ames. 
```
...
population mean.
```
95% of the students would capture the true population mean. 
```
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```
    ```
    About 92% of the interval include the true mean. 
    It is not exactly equal to the confidence level. 
    95% confidence interval means that at least 95% of the interval will include the true mean.
    ```

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    ```
    I choose the 99% confidence interval. The critical value is +/-(2.58).
    ```
