---
title: "Introduction to data - DS606 Lab1"
author: "Qixing Li"
output:
  html_document:
    theme: cerulean
...
3.  What does the mosaic plot reveal about smoking habits and gender?<br>
    The mosaic plot reveals that fewer females smoked 100 cigarettes in their lifetime than men.
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    **Base on the plot below, most of the data points lie under the red line indicate that most of
    the people desire lower weight. As the actual weight increase, more and more people desire
    lower weight.**
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    **The data type of the variable `wdiff` is numeric and it is continuous. However, what we 
    mainly concern is the number of how many people want to gain weight or lose weight. 
    If an observation is 0, it means the person' weight is equal to the desired weigth. If `wdiff` 
    is positive, then the person is above the desired weight, otherwise below the desired weight.**
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    **The distribution of `wdiff` is a little bit right skewed. From the plot we can see that more 
    peolpe want to lose weight than gain weight, same as the conclusion made above.**
    ```{r}
    ggplot(cdc,aes(wdiff))+
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    **It can see that more men want to gain weight than women do.** 
    ```{r}
    cdc$wdiff_d <- case_when(cdc$wdiff>0 ~ 'gain',
                             cdc$wdiff<0 ~ 'lose',
                             cdc$wdiff==0 ~ 'none')
    table(cdc$gender,cdc$wdiff_d)
    ```
    
    ```{r}
    #boxplot
    ggplot(cdc,aes(gender,wdiff))+
    geom_boxplot()
    ```
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    print(paste0("The mean of weight is: ", mean_w))
    ``` 
    
    ```{r}
    # standard deviation of `weight`
    sd_w <- sd(cdc$weight)
    print(paste0("The standard deviation of weight is: ", sd_w))
    ``` 
    
    ```{r}
    # proportion of the weights are within one standard deviation of the mean
    prop_one <- sum(cdc$weight> (mean_w -sd_w) & cdc$weight < (mean_w +sd_w)) / 20000
    print(paste0("The proportion of the weights are within one standard deviation of the mean is: ", prop_one))
    ```
