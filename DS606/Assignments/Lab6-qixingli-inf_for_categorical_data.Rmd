...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
    ```
    The smapling method should be random. 
    It might be difficult to become random in the global report
    because different countries might have different diversities.
    ```
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    ```
    Each row of Table 6 corresponds to a country and their percentages in 
    each category, while each row of atheism corresponds to each individual 
    and is only concerned about atheist and non-atheist. 
    ```
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    # The computed value from the atheism data fram agrees with the table 6 value which is 5%.
    ```
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    ```
    The observations are independent and the sample size is large enough.
    ```
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    inference(china12$response, est = "proportion", type = "ci", 
    ...
    inference(france12$response, est = "proportion", type = "ci", method="theoretical", success = "atheist")
    ```
...
8.  Describe the relationship between `p` and `me`.
    ```
    When p increases from 0 to .5, me also increases.  
    While p increases from -5 to 1.0, me decreases.
    ```
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    ```
    There is no significant different in the margin of error between 
    Australia and Ecuador. Therefore, it is ok to procees and make an 
    inference.
    ```
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    ```{r}
    sp2005 <- subset(atheism, nationality == "Spain" & year == "2005")
    sp2012 <- subset(atheism, nationality == "Spain" & year == "2012")
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    One would expect to interpret correctly a change 95% of the time, 
    and not so 5% of the time due to chance.
    ```
