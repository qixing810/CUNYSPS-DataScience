...
From the summary of the two data sets, we can clearly see that the *present* has much large numbers of newborns
```{r}
# show summary of the two data sets
summary(arbuthnot)
summary(present)
```
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
From the horizontal line in the plot below, we can see that the boy-to-girl ratio is greater than 1 for all time.

```{r}
# Boy-to-girl ratio in U.S.
ggplot(present,aes(year,boys/girls))+
  geom_line()+
  geom_hline(yintercept = 1.04,color='red')
```
...
```{r}
# Calculate the total newborns for ever year
present$total = present$boys + present$girls

# select the largest total row
